...
$message     = '';
$messageType = '';
$nom = $prenom = $email = $departement = $diplomes = $password = $tel = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validations
    if (empty($nom) || empty($prenom) || empty($email) || empty($departement) || empty($password)) {
        ...
        // Vérif unicité email (enseignant + étudiant)
        $c1 = $pdo->prepare("SELECT id FROM enseignant WHERE email = ?"); $c1->execute([$email]);
        $c2 = $pdo->prepare("SELECT id FROM etudiant   WHERE email = ?"); $c2->execute([$email]);
        if ($c1->fetch() || $c2->fetch()) {
...
<body>
  ...
  <nav>
    ...
    <div class="nav-links">
      ...
      <a href="gestion_enseignant.php">Gestion enseignants</a>
  ...
  <main class="page-wrapper">
    ...
    <div class="flash <?= $messageType ?>">
      ...
      <?php if ($messageType === 'success'): ?>
        &nbsp;—&nbsp;<a href="gestion_enseignant.php" style="color:var(--teal);font-weight:700">Voir la liste</a>
        &nbsp;|&nbsp;<a href="ajout_enseignant.php" style="color:var(--teal);font-weight:700">Ajouter un autre</a>
      <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="glass-panel">
      ...
      <form method="POST" action="ajout_enseignant.php" novalidate>
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="text" name="nom" placeholder="Ex : Moreau" value="<?= htmlspecialchars($nom) ?>" required>
          </div>
          <div class="form-group">
            <label>Prénom <span class="req">*</span></label>
            <input type="text" name="prenom" placeholder="Ex : Julian" value="<?= htmlspecialchars($prenom) ?>" required>
          </div>
