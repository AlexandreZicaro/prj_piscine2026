...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_cours') {
        ...
        $stmt = $pdo->prepare("INSERT INTO cours (nom_cours, description, capacite_max, id_enseignant, id_administrateur) VALUES (?,?,?,?,?)");
        $stmt->execute([$nom, $description, $capacite, $id_ens, $_SESSION['user_id']]);
        $message = "✅ Cours \"$nom\" créé avec succès.";
        $messageType = 'success';
    }
    ...
    elseif ($action === 'edit_cours') {
        ...
        $capacite    = intval($_POST['capacite_max']);
        $id_ens      = intval($_POST['id_enseignant']);
        $stmt = $pdo->prepare("UPDATE cours SET nom_cours=?, description=?, capacite_max=?, id_enseignant=? WHERE id_cours=?");
        $stmt->execute([$nom, $description, $capacite, $id_ens, $id]);
        $message = "✅ Cours \"$nom\" modifié avec succès.";
        $messageType = 'success';
    }
    ...
    elseif ($action === 'delete_cours') {
        $id = intval($_POST['id_cours']);
        $stmt = $pdo->prepare("SELECT nom_cours FROM cours WHERE id_cours=?");
        $stmt->execute([$id]);
        $nom = $stmt->fetchColumn();
        $pdo->prepare("DELETE FROM cours WHERE id_cours=?")->execute([$id]);
        $message = "🗑️ Cours \"$nom\" supprimé.";
        $messageType = 'danger';
    }
    ...
    elseif ($action === 'affecter_prof') {
        ...
        $id_ens   = intval($_POST['id_enseignant']);
        $stmt = $pdo->prepare("UPDATE cours SET id_enseignant=? WHERE id_cours=?");
        $stmt->execute([$id_ens, $id_cours]);
        $message = "✅ Enseignant affecté au cours avec succès.";
        $messageType = 'success';
    }

    // Inscrire un étudiant à un cours (via inscription)
    elseif ($action === 'inscrire_etudiant') {
        $id_cours   = intval($_POST['id_cours']);
        $id_etudiant = intval($_POST['id_etudiant']);
        // Récupère l'enseignant du cours
        ...
        if ($check->fetchColumn() > 0) {
            $message = "⚠️ Cet étudiant est déjà inscrit à ce cours.";
            $messageType = 'warning';
        } else {
            $stmt = $pdo->prepare("INSERT INTO inscription (date_inscription, id_etudiant, id_enseignant) VALUES (CURDATE(),?,?)");
            $stmt->execute([$id_etudiant, $id_ens]);
    ...
    elseif ($action === 'desinscrire_etudiant') {
        $id_insc = intval($_POST['id_inscription']);
        $pdo->prepare("DELETE FROM inscription WHERE id_inscription=?")->execute([$id_insc]);
        $message = "✅ Étudiant désinscrit du cours.";
        $messageType = 'success';
    }
}
...
$inscrits = [];
$stmtInsc = $pdo->query("
    SELECT i.id_inscription, i.id_etudiant, i.id_enseignant, e.nom, e.prenom, e.classe
    FROM inscription i
    JOIN etudiant e ON i.id_etudiant = e.id
");
foreach ($stmtInsc->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $inscrits[$row['id_enseignant']][] = $row;
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-logout:hover { background: var(--alert); color: #fff; }
    ...
    .btn-add { background: linear-gradient(135deg, var(--teal), #00b4d8); color: #fff; border: none; border-radius: 10px; padding: 12px 24px; font-family: 'Syne', sans-serif; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; text-decoration: none; transition: transform .2s, box-shadow .2s; }
    .btn-add:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(10,147,150,.3); }
    ...
    th { font-family: 'Syne', sans-serif; font-size: 0.78rem; font-weight: 700; text-transform: uppercase; color: #3d5a5a; padding: 10px 12px; border-bottom: 2px solid rgba(13,27,42,0.15); text-align: left; }
    td { padding: 12px; border-bottom: 1px solid rgba(13,27,42,0.07); font-size: 0.92rem; vertical-align: middle; }
    tr:last-child td { border-bottom: none; }
    tr:hover td { background: rgba(255,255,255,0.5); }

    .badge-ens { display: inline-block; background: rgba(10,147,150,0.12); color: var(--teal); padding: 3px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; }
    .badge-none { display: inline-block; background: rgba(230,57,70,0.1); color: var(--alert); padding: 3px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; }
    .badge-count { display: inline-block; background: rgba(13,27,42,0.08); color: var(--navy); padding: 3px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; }
    ...
    .btn-action.edit   { color: var(--teal); background: rgba(10,147,150,0.1); }
    .btn-action.edit:hover { background: var(--teal); color: #fff; }
    .btn-action.danger { color: var(--alert); background: rgba(230,57,70,0.1); }
    .btn-action.danger:hover { background: var(--alert); color: #fff; }

    /* DETAIL PANEL */
    .detail-panel { display: none; }
    .detail-panel.open { display: block; }
    .detail-title { font-family: 'Syne', sans-serif; font-size: 1.3rem; font-weight: 800; color: var(--navy); margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 2px solid rgba(13,27,42,0.08); display: flex; justify-content: space-between; align-items: center; }
    .detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; }
    @media (max-width: 800px) { .detail-grid { grid-template-columns: 1fr; } }

    .section-label { font-family: 'Syne', sans-serif; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; color: #6b8080; margin-bottom: 0.8rem; letter-spacing: 0.05em; }

    /* FORMULAIRES INLINE */
    .inline-form { display: flex; gap: 0.8rem; align-items: center; flex-wrap: wrap; margin-bottom: 1rem; }
    .form-select { background: rgba(255,255,255,0.9); border: 1.5px solid #cce0e0; border-radius: 10px; padding: 10px 14px; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; color: var(--navy); outline: none; flex: 1; min-width: 180px; transition: border-color .2s; }
    .form-select:focus { border-color: var(--teal); }
    .btn-submit { background: linear-gradient(135deg, var(--teal), #00b4d8); color: #fff; border: none; border-radius: 10px; padding: 10px 20px; font-family: 'Syne', sans-serif; font-weight: 700; font-size: 0.88rem; cursor: pointer; white-space: nowrap; transition: transform .2s; }
    .btn-submit:hover { transform: translateY(-1px); }

    /* LISTE INSCRITS */
    .inscrits-list { display: flex; flex-direction: column; gap: 8px; max-height: 280px; overflow-y: auto; }
    .inscrit-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; background: rgba(255,255,255,0.6); border-radius: 10px; border-left: 3px solid var(--teal); }
    .inscrit-name { font-weight: 700; font-size: 0.9rem; }
    .inscrit-classe { font-size: 0.78rem; color: #6b8080; margin-top: 2px; }
    .btn-rm { background: none; border: none; color: var(--alert); font-size: 1rem; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: background .2s; }
    .btn-rm:hover { background: rgba(230,57,70,0.12); }
    .inscrits-empty { color: #6b8080; font-style: italic; font-size: 0.88rem; padding: 0.5rem 0; }

    /* DANGER ZONE */
    .danger-zone { border-top: 2px dashed rgba(230,57,70,0.3); padding-top: 1rem; margin-top: 1rem; }
    .btn-danger { background: var(--alert); color: #fff; border: none; border-radius: 10px; padding: 10px 20px; font-family: 'Syne', sans-serif; font-weight: 700; cursor: pointer; width: 100%; transition: opacity .2s; }
    ...
    .form-group input, .form-group select, .form-group textarea { padding: 10px 14px; border: 1.5px solid #cce0e0; border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 0.92rem; color: var(--navy); background: #f0fafa; outline: none; transition: border-color .2s; }
...
<body>
  ...
  <main class="page-wrapper">
    ...
    <div class="glass-panel">
      <table>
        ...
        <tbody>
          <?php if (empty($cours)): ?>
          <tr><td colspan="6" style="text-align:center;color:#6b8080;font-style:italic;padding:2rem">Aucun cours créé pour le moment.</td></tr>
          <?php else: ?>
          <?php foreach ($cours as $c):
            $nbInscrits = isset($inscrits[$c['id_enseignant']]) ? count($inscrits[$c['id_enseignant']]) : 0;
          ?>
          <tr style="cursor:pointer" onclick="openDetail(<?= htmlspecialchars(json_encode($c)) ?>, <?= htmlspecialchars(json_encode($inscrits[$c['id_enseignant']] ?? [])) ?>)">
            <td><strong><?= htmlspecialchars($c['nom_cours']) ?></strong></td>
            <td style="color:#4a6a6a;font-size:0.85rem"><?= htmlspecialchars($c['description'] ?? '—') ?></td>
            <td><?= $c['capacite_max'] ?? '—' ?></td>
            <td>
              <?php if ($c['ens_nom']): ?>
                <span class="badge-ens"><?= htmlspecialchars($c['ens_prenom'] . ' ' . $c['ens_nom']) ?></span>
              <?php else: ?>
                <span class="badge-none">Non affecté</span>
              <?php endif; ?>
            </td>
            <td><span class="badge-count"><?= $nbInscrits ?> étudiant<?= $nbInscrits > 1 ? 's' : '' ?></span></td>
            <td onclick="event.stopPropagation()" style="white-space:nowrap">
              <button class="btn-action edit" onclick="openModalEdit(<?= htmlspecialchars(json_encode($c)) ?>)">✏️ Modifier</button>
              <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer ce cours définitivement ?')">
    ...
    <div class="glass-panel detail-panel" id="detailPanel">
      ...
      <div class="detail-grid">
        ...
        <!-- COLONNE GAUCHE : affecter enseignant -->
        <div>
          ...
          <form method="POST" class="inline-form">
            ...
            <select name="id_enseignant" class="form-select" required>
              ...
              <option value="<?= $e['id'] ?>" id="ens-opt-<?= $e['id'] ?>">
                <?= htmlspecialchars($e['prenom'] . ' ' . $e['nom']) ?> — <?= htmlspecialchars($e['departement']) ?>
              </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="section-label" style="margin-top:1.5rem">🎓 Inscrire un étudiant</div>
          <form method="POST" class="inline-form">
            ...
            <select name="id_etudiant" class="form-select" required>
              ...
              <option value="<?= $et['id'] ?>">
                <?= htmlspecialchars($et['prenom'] . ' ' . $et['nom']) ?> — <?= htmlspecialchars($et['classe']) ?>
              </option>
              <?php endforeach; ?>
            </select>
        ...
        <!-- COLONNE DROITE : liste des inscrits + danger zone -->
        <div>
          <div class="section-label">📋 Étudiants inscrits</div>
          <div class="inscrits-list" id="inscritsList">
            <div class="inscrits-empty">Sélectionnez un cours pour voir les inscrits.</div>
          </div>
    ...
    <div class="modal-box">
      ...
      <form method="POST" class="form-grid">
        ...
        <div class="form-group full-width">
          ...
          <input type="text" name="nom_cours" id="form-nom" required placeholder="Ex: Bases de données NoSQL">
        </div>
        <div class="form-group full-width">
          <label>Description</label>
          <textarea name="description" id="form-desc" placeholder="Description du cours..."></textarea>
        </div>
        <div class="form-group">
          <label>Capacité maximale</label>
          <input type="number" name="capacite_max" id="form-cap" min="1" max="200" placeholder="Ex: 30">
        </div>
        <div class="form-group">
  ...
  <script>
    // Données PHP → JS pour les inscrits
    ...
    function openDetail(cours, inscrits) {
      ...
      // Pré-sélectionner l'enseignant actuel
      const selEns = document.querySelector('[name="id_enseignant"].form-select');
      ...
      const data = inscritsData[cours.id_enseignant] || [];
      if (!data.length) {
        list.innerHTML = '<div class="inscrits-empty">Aucun étudiant inscrit.</div>';
      } else {
        list.innerHTML = data.map(i => `
          <div class="inscrit-item">
            <div>
              <div class="inscrit-name">${i.prenom} ${i.nom}</div>
              <div class="inscrit-classe">${i.classe}</div>
            </div>
            <form method="POST" style="margin:0" onsubmit="return confirm('Désinscrire cet étudiant ?')">
              <input type="hidden" name="action" value="desinscrire_etudiant">
              <input type="hidden" name="id_inscription" value="${i.id_inscription}">
              <button type="submit" class="btn-rm" title="Désinscrire">✕</button>
            </form>
          </div>`).join('');
      }
    ...
    function openModal() {
      ...
      document.getElementById('form-action').value = 'create_cours';
      document.getElementById('form-id').value = '';
      document.getElementById('form-nom').value = '';
      document.getElementById('form-desc').value = '';
      document.getElementById('form-cap').value = '';
      document.getElementById('courseModal').classList.add('open');
    }

    function openModalEdit(cours) {
      document.getElementById('modalTitle').textContent = 'Modifier le cours';
      document.getElementById('form-action').value = 'edit_cours';
      document.getElementById('form-id').value = cours.id_cours;
      document.getElementById('form-nom').value = cours.nom_cours;
      document.getElementById('form-desc').value = cours.description || '';
      document.getElementById('form-cap').value = cours.capacite_max || '';
      document.getElementById('form-ens').value = cours.id_enseignant || '';
      document.getElementById('courseModal').classList.add('open');
    }
